...
class ProcessEpisodeVideoUpload implements ShouldQueue
{
     ...
     * job. Dua tempat di jalur unggah membacanya — kolom `uploaded_by` di
     * `episode_videos` dan `user_id` di `activity_logs` — dan keduanya akan
     * kosong untuk setiap unggahan yang lewat antrean. Riwayat yang tidak bisa
     * menjawab "siapa yang mengunggah ini" kehilangan sebagian besar gunanya.
     *
     * Cara ini dipilih daripada menambah parameter aktor ke
     * `EpisodeVideoService`: modul unggah video Sprint 7.5 jadi tidak perlu
     * diubah sama sekali, dan `ActivityLogger` yang dipakai SELURUH panel juga
     ...
     * barisnya akan tertinggal berstatus PROCESSING selamanya — terlihat
     * seperti unggahan yang masih berjalan padahal sudah mati.
     */
