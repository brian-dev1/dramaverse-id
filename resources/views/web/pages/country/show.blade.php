...
@php($countryName = strcasecmp((string) $country->name, 'Tiongkok') === 0 ? 'China' : $country->name)
@section('title', 'Drama '.$countryName)
@section('description', 'Drama asal '.$countryName.' dengan subtitle Bahasa Indonesia.')
...
@section('content')
    ...
    <section class="page-head section-pad">
        ...
        <h1 class="page-title"><x-web.home.country-badge :country="$country" :show-label="true" /></h1>
    </section>

    @if ($dramas->isEmpty())
        <x-web.home.empty-state
            title="Belum ada drama dari {{ $countryName }}"
